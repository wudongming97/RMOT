...
class Detector(object):
    def __init__(self, args, checkpoint_id=None, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'bdd100k', self.seq_num))
        img_list = [os.path.join(self.args.mot_path, 'bdd100k', self.seq_num, _) for _ in img_list if ('jpg' in _) or ('png' in _)]
        ...
        self.sentence = ['cars']
        # self.sentence = ['pedestrian']
        # self.sentence = ['the cars in the right']
        # self.sentence = ['the black cars']
        # self.sentence = ['the red cars in the right']
        # self.sentence = ['the pedestrian in the right']

        self.tr_tracker = MOTR()
        self.save_path = os.path.join(self.args.output_dir, '{}'.format(seq_num))
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100):
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            dt_instances = self.filter_dt_by_ref_scores(dt_instances, 0.1)
...
if __name__ == '__main__':
    ...
    #             'MOT17-12-SDP',
    #             'MOT17-14-SDP']
    data_root = '/data/Dataset/bdd100k/'
    # video_ids = sorted(os.listdir(expressions_root))
    # video_ids = ['0000f77c-6257be58', '00202076-9eaa8e42', '000e0252-8523a4a9',
    #              '002cd38e-c7defded', '002cd38e-ebe888e1', '002d290d-01969e7d',
    #              '0066b72f-974f6883', '006a7635-c42f9f97']
    video_ids = ['007aeb45-3e75ce0e']
    # video_ids = ['0011']
    # video_ids = ['MOT17-11-SDP']

    for video_id in video_ids:
        print('Evaluating seq {}'.format(video_id))
        det = Detector(args, checkpoint_id, model=detr, seq_num=video_id)
        det.detect()
