...
class ListImgDataset(Dataset):
    def __init__(self, img_list, seq_num) -> None:
        super().__init__()
    ...
    def load_img_from_file(self, f_path):
        ...
        json_path = os.path.join('/data/Dataset/refer-kitti/expression', self.seq_num[0], self.seq_num[1])
        with open(json_path) as f:
            json_info = json.load(f)
        frame_id = int(label_path.split('/')[-1].split('.')[0])
        boxes = []
        ids = []
        if str(frame_id) in json_info['label'].keys():
            refer_ids = json_info['label'][str(frame_id)]
            refer_ids = [int(refer_id) for refer_id in refer_ids]
            for box, id in zip(targets['boxes'], targets['id']):
                if id in refer_ids:
                    boxes.append(box)
                    ids.append(id)
            img_show = draw_bboxes(torch.tensor(cur_img), boxes, ids)
        else:
            img_show = cur_img
        return cur_img, targets, img_show

    def init_img(self, img, img_show):
        ori_img = img.copy()
        ...
        img = img.unsqueeze(0)
        return img, ori_img, img_show

    def __len__(self):
        return len(self.img_list)

    def __getitem__(self, index):
        img, targets, img_show = self.load_img_from_file(self.img_list[index])
        return self.init_img(img, img_show)
...
class Detector(object):
    def __init__(self, args, checkpoint_id=None, model=None, seq_num=2):
        ...
        self.save_path = os.path.join(self.args.output_dir, '{}/{}'.format(seq_num[0], seq_num[1].split('.')[0]))
    ...
    @staticmethod
    def visualize_img_with_bbox(img_path, img, gt_boxes=None):
        if gt_boxes is not None:
            img_show = draw_bboxes(img, gt_boxes, identities=np.ones((len(gt_boxes),)) * -1)
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100):
        ...
        loader = DataLoader(ListImgDataset(self.img_list, self.seq_num), 1, num_workers=2)
        for i, (cur_img, ori_img, img_show) in enumerate(tqdm(loader)):

            img_path = os.path.join(self.save_path, 'frame_{}.jpg'.format(i))
            cv2.imwrite(img_path, cv2.cvtColor(img_show[0].numpy(), cv2.COLOR_RGB2BGR))
