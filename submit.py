...
class Detector(object):
    def __init__(self, args, checkpoint_id, model=None, seq_num=2):

        self.args = args
        self.detr = model

        self.seq_num = seq_num
        img_list = os.listdir(os.path.join(self.args.mot_path, 'MOT17/images/train', self.seq_num[0], 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'MOT17/images/train', self.seq_num[0], 'img1', _) for _ in img_list if
                    ('jpg' in _) or ('png' in _)]

        self.img_list = sorted(img_list)
        self.img_len = len(self.img_list)

        self.json_path = os.path.join('/data/Dataset/MOT17/expression_clean/', seq_num[0], seq_num[1])
        ...
        self.save_path = os.path.join(self.args.output_dir, 'results_epoch{}_/{}/{}'.format(checkpoint_id, seq_num[0], seq_num[1].split('.')[0]))
    ...
    @staticmethod
    def filter_dt_by_ref_scores(dt_instances: Instances, score_threshold: float) -> Instances:
        keep = dt_instances.ref_scores > score_threshold
        return dt_instances[keep]
    ...
    def write_gt(txt_path, json_file, gt_txt_file, im_height, im_width):
        ...
        with open(txt_path, 'w') as f:
            for k in json_info['label'].keys():
                frame_id = int(k) + 1
                frame_gt = np.loadtxt(
                    os.path.join(gt_txt_file, 'img1', '{:06d}.txt'.format(frame_id)))
                for frame_gt_line in frame_gt:
                    ...
                    if int(frame_gt_line[1]) in aa:  # choose referent gt from all gt
                        ...
                        x0, y0, w, h = frame_gt_line[2:6]
                        x1, y1 = x0 - w / 2, y0 - h / 2
                        x2, y2 = x0 + w / 2, y0 + h / 2
                        line = save_format.format(frame=frame_id, id=track_id, x1=x1*im_width, y1=y1*im_height, w=w*im_width, h=h*im_height)
                        f.write(line)
                    # f.write(line)

        # save all objects
        # with open(txt_path, 'w') as f:
        #     gt_txt_file = os.path.join(gt_txt_file, 'img1')
        #     frame_gt_files = os.listdir(gt_txt_file)
        #     frame_gt_files = sorted(frame_gt_files)
        #     for frame_gt_file in frame_gt_files:
        #         frame_id = int(frame_gt_file.split('.')[0])
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100):
        ...
        self.write_gt(gt_path, self.json_path, os.path.join('/data/Dataset/MOT17/labels_with_ids/train/', self.seq_num[0]),
                      im_height, im_width)
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            res = self.detr.inference_single_image(cur_img.cuda().float(), self.sentence, (seq_h, seq_w), track_instances)
...
if __name__ == '__main__':
    ...
    #             'MOT17-12-SDP',
    #             'MOT17-14-SDP']
    det_root = '/data/wudongming/FairMOT'
    data_root = '/data/Dataset/MOT17/images/train/'
    expressions_root = '/data/Dataset/MOT17/expression_clean/'
    # video_ids = sorted(os.listdir(expressions_root))
    video_ids = ['MOT17-09-SDP', 'MOT17-11-SDP']
